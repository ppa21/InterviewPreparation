/*
    ...
    * VARIANT: Implement del(key) and last() functions
*/
class LRUCache {
    ...
    // delete the node based on the key
    ...
    // get the last node
    public int last() {
        if (this.right.prev == this.left) { // list empty -> only dummy nodes
            return -1;
        }
        return this.right.prev.val; // The node before the right dummy is the MRU
    }
